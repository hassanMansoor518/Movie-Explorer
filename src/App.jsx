...
const MovieApp = () => {
  ...
  // Load movies when active tab changes
  useEffect(() => {
    if (!query.trim()) {
        ...
        {!selectedMovie && (
          <div className="w-full md:flex-1 flex justify-center">
            <input
              ...
              className="px-6 py-3 rounded-full bg-gray-900 text-gray-300 w-full max-w-2xl border border-gray-700 focus:outline-none focus:ring-2 focus:ring-yellow-400"
            />
          </div>
        )}
      </div>
          ...
          <div className="relative flex justify-center items-center bg-black md:h-[400px] h-[220px]">
            {trailer ? (
              <div className="w-full flex justify-center mt-10">
                <iframe
                  className="w-[90%] md:w-[60%] aspect-video rounded-lg shadow-lg"
            ...
            <button
              ...
              className="absolute top-3 left-3 md:top-4 md:left-4 bg-yellow-400 text-black px-3 py-1 md:px-4 md:py-2 rounded-lg font-bold text-sm md:text-base hover:bg-yellow-300 transition"
            >
              ← Back
            </button>
          </div>

          <div className="py-8 max-w-5xl mx-auto px-4 text-center md:text-left mt-10">
            ...
            <p className="text-yellow-400 font-bold mb-4 text-sm md:text-base">
              ⭐ Rating:{" "}
            ...
            {movies.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                {movies.map((movie) => (
                  <div
                    key={movie.id}
                    className="bg-gray-900 rounded-lg shadow-lg overflow-hidden relative group cursor-pointer hover:scale-105 transition-transform duration-300"
                    onClick={() =>
